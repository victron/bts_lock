...
# v.1.5.0	options start to use; loop through the positional parameters
# v.1.5.0	options ready to use
# v.1.6.0	check on Mu BTS_IP with expected IP
#**************************************************
#    constants block
#**************************************************
...
# <program_name>.process_number.<file_memplate  (bts_lock.sh.26051.commands.tmp)
 ...
 mu_output="./$(basename $0).$$.mu_output.tmp"
...
delete_tmp_files ()
  ...
  echo " start delete tmp files"
    ...
    rm $mu_output && echo "deleted.. $mu_output"
    return 0
  }
...
# parameter $1 shut, noshut, shut_ser, shut_mu, noshut_ser, noshut_mu
  {
...
# check BTS_Name presents on ASR
# set in global $hostname_1 - ASR hostname
# 
# $1 - <bts_name>
  {
  echo "debug> enter in  search_host_name" 
# Allow only capital leters and numbers in BTS_Name  
  if [[ $1 =~ ^[A-Z0-9]+$ ]]
    then 
      echo " searching \"$1\" in *.db"
...
# $3 number of ping to send
  {
  # collect info about Mu interface
  echo "sh ppp multil int multil $2" | ssh -T $1 > $mu_output
...
#    BTS_IP=$(echo "sh ppp multil int multil $2" | ssh -T $1 | awk '/Bundle name:/ {print $3}'| tr -dc '[:print:]')
     BTS_IP=$(awk '/Bundle name:/ {print $3}' $mu_output | tr -dc '[:print:]')
     Mu_IP=$(awk '/Local Endpoint/ {print $5}' $mu_output | tr -dc '[:print:]')
     # arithmetic done inside awk
     BTS_IP_expected=$(echo $Mu_IP | awk -F. '{$5=$4+1; print $1"."$2"."$3"."$5}')
      
echo "debug>  start ping BTS_IP= $BTS_IP"
    # check on null value
    if [[ -n "$BTS_IP" ]]
      then
	  if [[ $BTS_IP = $BTS_IP_expected ]]
	  then
	    echo " BTS_IP == BTS_IP_expected ($BTS_IP = $BTS_IP_expected)"
	      else
		echo " >>>>>>>>>> warning!!!! BTS_IP != BTS_IP_expected ($BTS_IP != $BTS_IP_expected) <<<<<<<<<<"
		echo "`date +%Y-%m-%d_%H:%M:%S`-$$ err: $BTS_Name_glob has foreign IP on Mu$2 BTS_IP=$BTS_IP BTS_IP_expected=$BTS_IP_expected" >> `date +%Y-%m-%d`--bts_lock.log
	  fi
	...
	echo " $3 sec. silence to get proper output on sdtout........"
        ...
        echo "`date +%Y-%m-%d_%H:%M:%S` --- $BTS_Name_glob $BTS_IP $ping_result"
        echo "`date +%Y-%m-%d_%H:%M:%S`-$$ ping: $BTS_Name_glob $BTS_IP $ping_result" >> `date +%Y-%m-%d`--bts_lock.log
        
	  else
	    echo "`date +%Y-%m-%d_%H:%M:%S` >>>>> BTS ================== $BTS_Name_glob ================== do not have IP"
	    echo "`date +%Y-%m-%d_%H:%M:%S`-$$ err: $BTS_Name_glob do not have IP" >> `date +%Y-%m-%d`--bts_lock.log
	    return 100
    fi
...
return 0
  ...
  { 
#    echo " >>>>>>>>>> below options currently not works <<<<<<<<<<<<<<<<"
    echo -e "Usage: $0 [-w <waite time>] [-m only Mu reset 5 sec.]  [-M <Mu waite time>] \n [-P number of pings] <BTS_Name_1>..<BTS_Name_N>" 1>&2
    echo "Usage: $0 [-p 20 pings only] [-P number of pings] <BTS_Name_1>..<BTS_Name_N>" 1>&2
    echo "Usage: $0 [-v show versionand exit] " 1>&2
    echo "Usage: $0 [-h show this help] " 1>&2
    exit 1 
   }
   getopts_fun ()
   {
echo "debug => enter in getopts_fun"
      while getopts "h?w:M:mP:pv" options; do
      case "$options" in
        w)
            waite_time=${OPTARG}
            if [[ $waite_time =~ ^[0-9]+$ ]]
	...
	P)
	    ping_numbers=${OPTARG}
	      if [[ $ping_numbers =~ ^[0-9]+$ ]]
	      then 
		 echo " number of ping set to = $ping_numbers"
		 else
		    echo -e " -P need integer argument \n exit...1"
		    exit 1
	      fi
            ;;
	v)
	    grep script_version $0 | grep -v "grep"
	    exit 0
	    ;;
        M)
            waite_time_mu=${OPTARG}
	      if [[ $waite_time_mu =~ ^[0-9]+$ ]]
	      then 
		 echo " waite time multilink only= $waite_time_mu"
		 else
		    echo -e " -M need integer argument \n exit...1"
		    exit 1
	      fi
            ;;
        h|\?)
...
exe_function ()
# parameter $1 actions to do
# normal - sshutdow all interfaces, waite, noshut Se, waite, noshut Mu, waite, ping 
  {
echo "debug=> enter in exe_function"
    # -------------------- exe function -------------------------------
...
"normal")
    ...
    echo " shutdown phase variant= normal"
    ...
    echo "`date +%Y-%m-%d_%H:%M:%S`  === $BTS_Name_glob === waiting $waite_time sec. + $waite_time_mu sec. on Mu.s + 20 sec. ping "
    #sleep $waite_time
...
#    generate_command_list_shut_noShut noshut || check_exit_status "list_shutown error"
    ...
    echo "`date +%Y-%m-%d_%H:%M:%S`  === $BTS_Name_glob === waiting $waite_time_mu sec. "
    #sleep $waite_time
    waite_function $waite_time_mu
    generate_command_list_shut_noShut noshut_mu || check_exit_status "list_shutown (Mu) error"
    # 5-th connection
    send_command $hostname_1 $command_list_shut_file || check_exit_status "ssh error"
    ;;
# below cases is clones of above, but with different algorithms
"Mu")
     # ---------------------------- action -----------------------------
    # ----------------- shutdown ----------------
    # generates command list 'shutdown' in $command_list_shut_file only multilink
    echo " shutdown phase variant= normal"
    generate_command_list_shut_noShut shut_mu || check_exit_status "list_shutown error"
    # 3-d connection
    # send commands from $command_list_shut_file
    #off
    send_command $hostname_1 $command_list_shut_file || check_exit_status "ssh error"
    ...
    # ----------- phase  ------ waiting ----------------
    echo "`date +%Y-%m-%d_%H:%M:%S`  === $BTS_Name_glob === waiting only Mu $waite_time_mu sec. + 20 sec. ping "
    #sleep $waite_time
    waite_function $waite_time_mu
...
"Ping")
    # ping only
    ping_bts $hostname_1 $Mu_num $ping_numbers
    echo "`date +%Y-%m-%d_%H:%M:%S`   --------- end processing -----> $BTS_Name_glob --------- "
    delete_tmp_files
    return 0
    ;;
"Null")
    echo " exe_function null"
    return 0
    ;;
    esac
  ...
  algo_funtion ()
  # algorithms actions in different scenarios
  # parameter $1 actions to do
    {
    case "$1" in
"Null_A")
...
getopts_fun $@
echo "debug=> shiftin OPTIND $OPTIND"
# shift is used to change the positional parameter values after getopts
# now non getopts arguments start #1
shift $((OPTIND-1)) 
...
# loop through the positional parameters
    while [[ "$1" ]]; do
      BTS_Name_glob=$1
# construction cmd >>file.txt 2>&1 appends to file and stdout
      exe_function $exe_case || echo " `date +%Y-%m-%d_%H:%M:%S`-$$ err: BTS $BTS_Name_glob >>>>>>> not found in .db return.. 1"  >> ./`date +%Y-%m-%d`--bts_lock.log 2>&1
      shift
    done
    
    else
echo "debug=> read from stdin"    
      while read BTS_Name_glob; do
echo "debug=> read from stdin2"
...
echo "`date +%Y-%m-%d_%H:%M:%S` ##################### current process summary ping report (on current date) #############################"
grep $$ ./`date +%Y-%m-%d`--bts_lock.log
echo "`date +%Y-%m-%d_%H:%M:%S` ##################### pid: $$ log file: ./`date +%Y-%m-%d`--bts_lock.log #############################"
